...
pygame.display.set_caption("Dijakstra Path Finding Algorithm")
...
def algorithm(draw,grid,start,end):
    ...
    while not open_set.empty():
        ...
        if current ==end:
            ...
            start.make_start()
            end_time = time.time()
            display_time_in_caption(end_time-start_time)
            return True
        ...
        for neighbor in current.neighbors:
            ...
            if temp_g_score < g_score[neighbor]:
                ...
                count+=1
                open_set.put((g_score,count,neighbor))
                neighbor.make_open()
...
def display_time_in_caption(duration):
    pygame.display.set_caption(f"Dijakstra Search Path Finding Algorithm - Time: {duration:.4f} seconds")
