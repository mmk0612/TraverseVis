...
def algorithm(draw, grid, start, end):
    ...
    while open_set:
        ...
        current = open_set.pop()  # Change to pop from the end of the list (stack behavior)
        if current == end:
...
def display_time_in_caption(duration):
    pygame.display.set_caption(f"Depth-First Search Path Finding Algorithm - Time: {duration:.4f} seconds")
