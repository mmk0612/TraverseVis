...
def algorithm(draw, grid, start, end):
    ...
    while open_set != end:
        ...
        for neighbor in current.neighbors:
            ...
            if neighbor not in open_set_hash and (next_spot is None or temp_h_score < h_score[next_spot]):
                next_spot = neighbor
                came_from[neighbor] = current
                open_set_hash.add(neighbor)

        if next_spot is current:
            break
    ...
    if open_set == end:
        ...
        display_time_in_caption(duration)
        return True
    
    else:
        pygame.display.set_caption(f"Hill Climbing Path Finding Algorithm - No Path Found")
        return False

def display_time_in_caption(duration):
    pygame.display.set_caption(f"Hill Climbing Path Finding Algorithm - Time: {duration:.4f} seconds")
